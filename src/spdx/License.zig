...
pub const Tree = struct {
    ...
    fn extractTokens(a: std.mem.Allocator, s: []const u8) !std.ArrayListUnmanaged(Token) {
    ...
    pub fn parse_expression(
        ...
        _ = s;
        var output: std.ArrayListUnmanaged(usize) = .empty;
        defer output.deinit(allocator);
        var operators: std.ArrayListUnmanaged(usize) = .empty;
        ...
        for (tokens.items, 0..) |token, i| {
            if (token.isOperator()) {
                ...
                try operators.append(allocator, i);
        ...
        while (operators.pop()) |token| try output.append(allocator, token);
        ...
        var stack: std.ArrayListUnmanaged(*TreeNode) = .empty;
        errdefer {
    ...
    pub fn new(a: std.mem.Allocator, s: []const u8) !@This() {
        var tokens = try extractTokens(a, s);
        errdefer tokens.deinit(a);
...
test "parse expression into tokens" {
    ...
    for (token_test_cases) |token_test| {
        var t = try Tree.new(a, token_test.expr);
        defer t.deinit();
...
test "generate tree #1" {
    ...
    var t = try Tree.new(a, "MIT AND (LGPL-2.1-or-later OR BSD-3-Clause)");
    defer t.deinit();
...
test "generate tree #2" {
    ...
    var t = try Tree.new(a, "(LGPL-2.1-or-later OR BSD-3-Clause) AND MIT");
    defer t.deinit();
...
test "generate tree #3" {
    ...
    var t = try Tree.new(a, "MIT AND LGPL-2.1-or-later OR BSD-3-Clause AND Apache-2.0");
    defer t.deinit();
